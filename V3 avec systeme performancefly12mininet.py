...
def monitor_resources(duration=30, interval=5):
    cpu_usage = []
    memory_usage = []
    timestamps = []
...
def main():
    ...
    # Monitor resources before attack in a separate thread
    ...
    memory_load_threads = [
        threading.Thread(target=simulate_memory_load, args=(client1, 30)),
        threading.Thread(target=simulate_memory_load, args=(client2, 30)),
        threading.Thread(target=simulate_memory_load, args=(client3, 30)),
        threading.Thread(target=simulate_memory_load, args=(client4, 30))
    ]
    ...
    # Collect resource usage data before and during attack
    timestamps_before, cpu_usage_before, memory_usage_before = monitor_resources(60, 5)
    timestamps_during, cpu_usage_during, memory_usage_during = monitor_resources(30, 1)

    # Plot resource usage
    plt.figure()
    plt.subplot(2, 1, 1)
    plt.plot(timestamps_before, cpu_usage_before, label='CPU Usage (Before DDoS)')
    plt.plot(timestamps_during, cpu_usage_during, label='CPU Usage (During DDoS)', linestyle='--')
    plt.xlabel('Time (seconds)')
    plt.ylabel('CPU Usage (%)')
    plt.legend()

    plt.subplot(2, 1, 2)
    plt.plot(timestamps_before, memory_usage_before, label='Memory Usage (Before DDoS)')
    plt.plot(timestamps_during, memory_usage_during, label='Memory Usage (During DDoS)', linestyle='--')
    plt.xlabel('Time (seconds)')
